...
def update_html(news_data, daily_content):
    ...
    if news_section:
        ...
        if news_sources and len(news_sources) >= 3:
            # 更新财联社新闻
            update_news_source(news_sources[0], news_data.get('cls', []))
            # 更新新浪财经新闻
            update_news_source(news_sources[1], news_data.get('sina', []))
            # 更新微博新闻
            update_news_source(news_sources[2], news_data.get('weibo', []))
...
def main():
    ...
    print(f"[OK] 财联社：{len(news_data.get('cls', []))} 条")
    print(f"[OK] 新浪财经：{len(news_data.get('sina', []))} 条")
    print(f"[OK] 微博：{len(news_data.get('weibo', []))} 条")
